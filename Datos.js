let datos = [
    ...
    ["1037817979","Luis Alberto Jaime Hernandez","Ingeniria  de Sistemas","II","4.0","4.1","3.9","4.0"],
...
function decimalALetras(decimal) {
...
function buscarEstudiante(id) {
    ...
    while (i < datos.length) {
        if (datos[i][0] === id) {
            ...
            let [entero, decimal] = promedioRedondeado.split('.').map(Number);
            let enteroEnLetras = numeroALetras(entero);
            let decimalEnLetras = decimalALetras(decimal);

            tablaResultados.innerHTML = `
                <tr>
                    ${fila.map(campo => `<td>${campo}</td>`).join('')}
                    <td>${promedioRedondeado} (${enteroEnLetras} punto ${decimalEnLetras})</td>
                </tr>`;
            return;
        }
        i++;
